...
// Marcadores que delimitan el bloque inyectado por este plugin. Permiten
// eliminarlo de forma exacta en pasadas repetidas (prebuilds que reusan ios/)
// sin depender de la estructura interna del código generado.
const TINCADIA_BLOCK_START = '// ── BEGIN TINCADIA VOIP BLOCK (generated by withVoipAppDelegate) ──';
const TINCADIA_BLOCK_END = '// ── END TINCADIA VOIP BLOCK ──';

// Los marcadores llevan paréntesis, que son metacaracteres de regex.
...
module.exports = function withVoipAppDelegate(config) {
  return withAppDelegate(config, (config) => {
    ...
    if (language === 'swift') {
      // Replace any previously injected PushKit block. Se hace por marcadores
      // explícitos y no por la forma del código: el bloque cambia de estructura
      // entre versiones del plugin (p. ej. al añadir la rama #else), y un regex
      // que dependa de esa forma deja de calzar y duplica símbolos en prebuilds
      // que reusan ios/ (error de compilación por redeclaración).
      contents = contents.replace(
        new RegExp(
          `\\n?${escapeRegExp(TINCADIA_BLOCK_START)}[\\s\\S]*?${escapeRegExp(TINCADIA_BLOCK_END)}\\n?`,
          'g'
        ),
        '\n'
      );

      // Bloques de versiones antiguas del plugin, anteriores a los marcadores.
      contents = contents.replace(
...
// Registro nativo del PKPushRegistry en cold start. Es una función libre (no un
// método de la extension) porque se invoca desde didFinishLaunchingWithOptions,
// que vive en el cuerpo de la clase AppDelegate: un método privado de la
// extension no forma parte de la API que ese punto de inyección puede resolver.
// El nombre debe calzar EXACTO con el que inyecta este plugin más arriba.
func TincadiaRegisterVoipPush() {
...
#else
// Sin PushKit no hay registry que registrar, pero la llamada inyectada en
// didFinishLaunchingWithOptions es incondicional: este no-op mantiene la
// compilación válida en cualquier configuración.
func TincadiaRegisterVoipPush() {}
#endif
${TINCADIA_BLOCK_END}
`;
